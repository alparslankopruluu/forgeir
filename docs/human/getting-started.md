...
`forge run` on the add example calls `add(2, 3)` and prints `5`. The clamp example prints `10`. The option demo prints `10`. The wrap demo prints `main.fir`. The env demo needs `--allow env`.
