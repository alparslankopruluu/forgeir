...
2. Language is M1 (records, `list`/`Option`/`Result`, `if`/`match`, calls). Addressability is M2 (lockfile, query/get, `replace_expr`). Do not add effects, `extern`, or backends unless the task says so.
