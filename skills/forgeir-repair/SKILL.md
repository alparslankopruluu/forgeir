...
3. Preview with `forge patch --qid <qid> --expr '...'` or `--rename <name>`, or MCP `forge_patch`. Apply only after the hunk looks right. Use `rename` (not a raw edit) so nids survive.
